  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <Controller
          ...
          render={({ field }) => (
            <input
              ...
              className={cn(
                `w-full py-3 px-5 rounded-3xl border-2 outline-none mb-6 transition`,
                errors.email?.message ? 'border-2 border-primary' : 'border-transparent'
              )}
            />
          )}
        />
      ...
      <div className="flex justify-end">
        <Button className="border-none hover:border-primary uppercase text-lg text-white" type="submit">
          Subscribe
        </Button>
      </div>
    </form>
